...
class MyGraphDataset(Dataset):
    def __init__(self, table_path,
                 dmri_graph_dir,
                 fmri_graph_dir,
                 data_type='train',fc_threshold=0,sc_threshold=0,fold=5,ROI=100, BOLD=False,is_NC_only=False,cls_type='AD',AD_types='AD_NC'):
        self.data_type = data_type
        if sc_threshold:
            self.dmri_graph_dir = dmri_graph_dir+ f'_filtered_0{int(sc_threshold*10)}_ROI{ROI}_pt'
        else:
            self.dmri_graph_dir = os.path.join(dmri_graph_dir+'_graph_pt',f'ROI{ROI}_pt')
        if fc_threshold:
            self.fmri_graph_dir = fmri_graph_dir + f'_AE_filtered_0{int(fc_threshold*10)}_ROI{ROI}_pt'
        else:
            self.fmri_graph_dir = os.path.join(fmri_graph_dir+'_AE',f'ROI{ROI}_pt')
        ...
        if is_NC_only:
            ...
            if cls_type == 'AD':
                self.table = self.table.dropna(subset=['Diag-GUO'])
                self.AD_types = AD_types.split('_')
                self.table = self.table[self.table['Diag-GUO'].isin(self.AD_types)]
            else:
                self.table = self.table.dropna(subset=['combine_encoder'])
                self.AD_types = None
        if data_type=='train':
    ...
    def get_fmri_data(self,fmri_graph_path,):
        ...
        edge_index = graph['edge_index']
        edge_attr = graph['edge_features'].reshape(-1,1)
        node_features = graph['node_features']
...
class MyGraphDataset_withPET(MyGraphDataset):
    def __init__(self, table_path,dmri_graph_dir, fmri_graph_dir,PET_dir,
                 data_type='train',fc_threshold=0,sc_threshold=0,fold=5,ROI=100, BOLD=False,classification=False,pick=False,cls_type='AD',AD_types='AD_NC'):
        ...
        self.PET_dir = PET_dir+f'_ROI{ROI}_pt2'
        self.table = pd.read_csv(table_path)
        # self.table = pd.read_csv(table_path)
        if data_type == 'train':
            self.table = self.table[self.table['fold'] != fold]
    ...
    def get_PET_data(self,PET_data_path):
        ...
        label = torch.load(PET_data_path).squeeze()
        return label
    def __getitem__(self, idx):
        ...
        pet_label = torch.tensor(torch.load(label_path)).squeeze()
        if self.classification:
            Abeta_label = self.table[self.table['subject_id'] == subject_id]['combine_encoder'].values[0]
            AD_label = self.table[self.table['subject_id'] == subject_id]['Diag-GUO'].values[0]
            # 处理Abeta_label的赋值情况
            if pd.isnull(Abeta_label):
                Abeta_label = 2
            elif Abeta_label == 0:
                Abeta_label = 0
            elif Abeta_label == 1:
            ...
            # 处理AD_label的赋值情况
            if self.AD_types == None:
                if pd.isnull(AD_label):
                    AD_label = 3
                elif AD_label == 'AD':
                    AD_label = 2
                elif AD_label == 'MCI':
                ...
                AD_label = self.AD_types.index(AD_label)
...
def collate_fn(batch_data,mask_ratio=None,fc=True,sc=True,seed =0):
    ...
    if 'PET_label' in batch_data[0].keys():
        ...
        if 'Abeta_label' in batch_data[0].keys():
            try:
                Abeta_labels = torch.tensor([i['Abeta_label'] for i in batch_data], dtype=torch.float32)
                AD_labels = torch.tensor([i['AD_label'] for i in batch_data], dtype=torch.float32)
            except:
                for j in batch_data:
                    print(j['subject_id'], j['AD_label'])
            return {
                'subject_ids': subject_ids,
                'SC_graphs': SC_graphs,
                'FC_graphs': FC_graphs,
                'PET_labels': PET_labels,
                'Abeta_labels': Abeta_labels,
                'AD_labels': AD_labels
            }
        return {
            'subject_ids': subject_ids,
